...
export const publishedAt = new Date(2025, 0, 16, 7);
...
_This is a series of articles about end-to-end encryption. You are currently reading part&nbsp;2. You can read part&nbsp;1 [here](/articles/e2ee-for-web-apps-1)._
...
We will see later if there is a better way to store the DEK. For now, serializing to base64 and storing in `localStorage` does the job.
...
When the user go to a new page, we retrieve the `iv` and the `encryptedData` from the backend and we decrypt data to show it to the user:
...
While our current implementation works, it still has flaws. In the next article we will learn about **KEK** (Key Encryption Key) and how it could improve our workflow.
